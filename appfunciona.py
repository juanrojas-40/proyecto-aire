...
if not archivos_csv:
    ...
    print(f"Se encontraron {len(archivos_csv)} archivos. Uniendo...")
    ...
    if listado_dataframes:
        df_unido = pd.concat(listado_dataframes, ignore_index=True)
        # Guardar el resultado
        df_unido.to_csv("openaq_todo_junto.csv", index=False)
        print("✅ Archivos unidos exitosamente en 'openaq_todo_junto.csv'")
    else:
        print("⚠️ No se pudo leer ningún archivo. Revisa el formato o codificación.")
